...
function searchquery($token, $dbId, $fld, $query) {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://192.168.1.214/api/v1/databases/" . $dbId . "/records?filter[query]=". $fld ."%20" . $query);
    ...
    <body>
        <div class="search">
            <form method="post">
                <!--
                <select name="idb">
                    <option selected value="425">БД -425 "Ресурсы Интернет"</option>
                </select>
                <select name="fld">
                    <option selected value="ID">Поле - ID</option>
